...
class VirtualQueueUseCase:
    ...
    def virtual_queue_endpoint(self, user: UserModel, id_web_session: str):
        ...
        print(f"Adicionando usuário {user.ID_USER} à fila virtual com sessão {id_web_session}")
        self.controller.add_to_queue(item)
        print(f"Usuário {user.ID_USER} está na posição {self.controller.get_queue_position(id_web_session)}º da fila virtual")
